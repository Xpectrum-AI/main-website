...
const FlowchartPage: React.FC = () => {
  ...
  const startFlowAnimation = () => {
    ...
    function showResponseMessage(x: number, y: number, color: string) {
      ...
      let scrollY = 0;
      const maxScroll = Math.max(0, yOffset - 460);
      
      const handleScroll = (event: WheelEvent) => {
        if (!flowComplete) return;
        event.preventDefault();
        scrollY = Math.max(0, Math.min(maxScroll, scrollY + event.deltaY * 0.5));
        responseText.style.transform = `translateY(${-scrollY}px)`;
      };
