...
const DOMAIN_CONFIG = {
  ...
  XpectrumDemo: {
    welcomeMessage: "Welcome to Xpectrum AI Demo! How can I help you today?",
    placeholderText: "Ask anything about our Agentic AI, voice, video, or CaaS..."
  }
};
...
const getApiKey = async (domain: keyof typeof DOMAIN_CONFIG = 'HRMS') => {
  try {
...
const getAssistantName = (domain: keyof typeof DOMAIN_CONFIG) => {
  switch(domain) {
    ...
    case 'HRMS':
      return 'HR Assistant';
    case 'XpectrumDemo':
      return 'Xpectrum AI Demo Assistant';
    default:
...
const ChatComponent: React.FC<ChatComponentProps> = ({ 
  ...
  // Add useEffect to fetch API key
  useEffect(() => {
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      try {
        ...
        console.log("Sending request to:", `${BASE_URL(domain)}/files/upload`);
        console.log("Using API key:", apiKey);
        
        const response = await fetch(`${BASE_URL(domain)}/files/upload`, {
          method: 'POST',
          headers: {
  ...
  const sendMessage = async (forcedQuery?: string) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL(domain)}/chat-messages`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${apiKey}`,
